...
const Sidebar = () => {
  ...
  return (
    <> 
    <div className="pt-9 hidden md:flex h-full flex-col bg-white border-r shadow:sm overflow-y-auto">
      {routes.map((route)=>{
        return <Link 
      ...
      {routes.map((route)=>{
        return <Link 
        className={cn("flex items-center flex-col justify-center text-slate-500 text-xs font-medium transition-all py-1 flex-1",
