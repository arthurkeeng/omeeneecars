...
import { getCars } from "@/actions/car-fetch";
import useFetch from "@/hook/use_fetch";
import { useRouter, useSearchParams } from "next/navigation";
import { useEffect, useState } from "react";
import CarsListingsLoading from "./cars-listing-loading";
...
const CarListing = () => {
  ...
  useEffect(() => {fetchCars({ search, make, bodyType,fuelType,transmission,
      minPrice, maxPrice, sortBy,page,limit, });
  }, [
   search, make, bodyType, fuelType, transmission, minPrice,
    ...
    if(cars.length === 0 ) {
      return <div className="flex min-h-[400px] flex-col items-center justify-center text-center p-8 border-rounded-lg bg-gray-50">
  ...
  <div className="flex items-center mb-6">
    <p className="text-gray-600 mx-2">Showing{" "}</p>
    <span className="mx-2">{(page - 1 ) * limit +1} - { Math.min(page * limit , pagination.total)} </span> {" "}
   {" "}  of <span className=" mx-2 font-medium"> {" "} {pagination.total}</span>{" "} cars
