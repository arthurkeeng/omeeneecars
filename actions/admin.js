...
export async function getAdminTestDrives(args) {
    try {

        // let {userId } = await auth()
        let { search = "", status = "" } = args[0]
        
        
        const isAdmin = await getAdmin();
        
        if (!isAdmin.authorized) throw new Error("Unauthorized");
        console.log('the is admin ' , isAdmin)
...
export async function updateTestDriveStatus(args) {
    try {
        let { bookingId, newStatus } = args[0]
